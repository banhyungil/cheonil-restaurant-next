...
```ts
interface CartItem { menuSeq: number; nm: string; price: number; cnt: number }
...
function onAddMenu(seq: number) {
  ...
  if (existing) { existing.cnt++; return }
  const menu = DUMMY_MENUS.find(m => m.seq === seq)!
  cart.value.push({ menuSeq: menu.seq, nm: menu.nm, price: menu.price, cnt: 1 })
}
function onIncrement(seq: number) { /* 찾아서 cnt++ */ }
function onDecrement(seq: number) { /* cnt--, 0이면 splice */ }
function onResetCart()           { cart.value = []; memo.value = '' }
function onCheckout()            { console.log('checkout', { cart, memo, selStore }) }
```
...
- Pinia `useOrderDraftStore` 로 카트 상태 승격
