...
| #   | 항목                                   | 우선순위 | 의존성                             |
| --- | -------------------------------------- | -------- | ---------------------------------- |
| 1   | Pinia `useOrderCartStore` 로 상태 승격 | 높음     | 없음                               |
...
파일 위치: `src/stores/orderCartStore.ts`
...
export const useOrderCartStore = defineStore(
  "orderDraft",
  () => {
    ...
    const cTotalCount = computed(() =>
      cart.value.reduce((s, i) => s + i.cnt, 0),
    );
    const cTotalAmount = computed(() =>
      cart.value.reduce((s, i) => s + i.price * i.cnt, 0),
    );
...
- `ref` 3개 제거, `useOrderCartStore()` 로 교체
...
// After
const orderDraft = useOrderCartStore()
...
export function getStoreCategoryTone(
...
<template>
  ...
  <span
    v-if="categoryName"
    :class="[
      'flex h-6 items-center rounded-xl px-2.5 text-sm font-semibold',
      cTone.bg,
      cTone.text,
    ]"
  >
...
- `useOrderCartStore` 에 `submit()` 메서드 추가 — 백엔드 `POST /api/orders` 호출
- 세트 메뉴 (디자인 확정 후)
- 모바일/태블릿 반응형
