...
-->
<template>
  <section class="flex flex-col h-full gap-5 px-8 py-6 voice-order-parse-example">
    <header class="flex items-center h-10 gap-3">
    ...
    <div class="flex flex-1 min-h-0 gap-5">
      ...
      <div class="flex flex-col w-1/2 gap-4">
        ...
        <div class="flex flex-col gap-2 p-3 border border-blue-200 rounded bg-blue-50/40">
          <div class="flex items-center justify-between">
            <span class="text-sm font-semibold text-blue-900"
              >음성 → 주문 생성 (fallback 테스트)</span
            >
        ...
        <div class="flex flex-col gap-2 p-3 border border-purple-200 rounded bg-purple-50/40">
          <div class="flex items-center justify-between">
            <span class="text-sm font-semibold text-purple-900"
              >STT 변환 비교 (Whisper vs Google)</span
            >
          </div>
          ...
          <div class="flex items-center gap-2">
            ...
            <span v-else-if="compareLoading" class="text-xs text-purple-700"
              >두 엔진 호출 중...</span
            >
          </div>
        </div>
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="flex flex-wrap gap-1.5">
            <button
              ...
              :key="s"
              type="button"
              class="px-2 py-1 text-xs rounded-md bg-surface-100 text-surface-700 hover:bg-surface-200"
              @click="userText = s"
            >
              {{ s }}
            </button>
        ...
        <div class="flex flex-col flex-1 min-h-0 gap-2">
          <span class="text-xs font-semibold text-surface-700"
            >매장 ({{ stores?.length ?? 0 }})</span
          >
          <div class="p-2 overflow-auto text-xs border rounded border-surface-200">
            <div v-for="s in stores" :key="s.seq">
              <span class="text-surface-400">[{{ s.seq }}]</span> {{ s.nm }}
            </div>
          </div>
          <span class="text-xs font-semibold text-surface-700"
            >메뉴 ({{ menus?.length ?? 0 }})</span
          >
          <div class="p-2 overflow-auto text-xs border rounded border-surface-200">
            <div v-for="m in menus" :key="m.seq">
              <span class="text-surface-400">[{{ m.seq }}]</span> {{ m.nm }} ({{
      ...
      <!-- 우측: 결과 -->
      <div class="flex flex-col w-1/2 gap-4 overflow-auto">
        <div v-if="error" class="p-3 text-sm text-red-700 border border-red-300 rounded bg-red-50">
          {{ error }}
        </div>
        ...
        <div
          v-if="compareResults"
          class="flex flex-col gap-2 p-3 text-sm border border-purple-200 rounded bg-purple-50/40"
        >
          ...
          <div class="grid grid-cols-2 gap-2">
            <div
              ...
              :key="r.engine"
              class="flex flex-col gap-1 p-2 bg-white border rounded border-surface-200"
            >
              <div class="flex items-center justify-between">
              ...
              <div v-else class="text-sm whitespace-pre-wrap wrap-break-word text-surface-900">
                {{ r.text }}
              </div>
              <button
                v-if="r.text"
        ...
        <div
          v-if="audioResult"
          class="flex flex-col gap-2 p-3 text-sm border rounded"
          :class="
        ...
        >
          <div class="flex items-center gap-2">
            <span class="text-xs font-semibold tracking-wide uppercase text-surface-700">
              STT 엔진
            </span>
            <span
        ...
        <div v-if="result" class="flex flex-col gap-3">
          <div>
            ...
            <div class="p-3 mt-2 text-sm border rounded border-surface-200 bg-surface-0">
              <div>
                <span class="text-surface-500">매장:</span>
                {{ resolvedStore?.nm ?? '(미지정)' }}
                <span v-if="result.storeSeq" class="text-xs text-surface-400"
                  >[seq={{ result.storeSeq }}]</span
                >
              </div>
              <div class="mt-2">
                <span class="text-surface-500">메뉴:</span>
          ...
          <details class="p-2 text-xs border rounded border-surface-200 bg-surface-50">
            <summary class="cursor-pointer text-surface-600">원본 응답 (claude raw)</summary>
            <pre class="mt-2 break-all whitespace-pre-wrap text-surface-700">{{ result.raw }}</pre>
          </details>
        </div>
      </div>
    </div>
